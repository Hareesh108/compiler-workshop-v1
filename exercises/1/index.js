...
function runTokenizerTests() {
  ...
  test('Tokenize with comments', () => {
    ...
    assert(tokens[0].type === 'CONST', 'First token should be CONST, comments are skipped');
  });
  ...
  test('Should throw on invalid characters', () => {
    ...
    assert(error.message.includes('Unexpected character'), 'Error message should mention unexpected character');
  });
}
...
function runParserTests() {
  ...
  test('Parse const declaration', () => {
    ...
    assertEqual(stmt.type, 'ConstDeclaration', 'Statement should be ConstDeclaration');
    assertEqual(stmt.id.name, 'x', 'Variable name should be "x"');
    assertEqual(stmt.init.type, 'NumericLiteral', 'Initializer should be NumericLiteral');
  ...
  test('Parse const with type annotation', () => {
    ...
    assertEqual(stmt.typeAnnotation.valueType, 'number', 'Type should be "number"');
  });
  ...
  test('Parse string literal', () => {
    ...
    assertEqual(init.value, 'hello world', 'String value should not include quotes');
  });
  ...
  test('Parse arrow function', () => {
    ...
    assertEqual(init.type, 'ArrowFunctionExpression', 'Should be ArrowFunctionExpression');
    ...
    assertEqual(init.body.type, 'BlockStatement', 'Body should be BlockStatement');
    assertEqual(init.body.body.length, 1, 'Body should have 1 statement');
    assertEqual(init.body.body[0].type, 'ReturnStatement', 'Statement should be ReturnStatement');
  });
  
  test('Parse arrow function with type annotations', () => {
    const tokens = tokenize('const add = (a: number, b: number): number => { return a + b; };');
    const ast = parse(tokens);
    const init = ast.body[0].init;
    
    assertEqual(init.params[0].typeAnnotation.valueType, 'number', 'First parameter should have number type');
    assertEqual(init.params[1].typeAnnotation.valueType, 'number', 'Second parameter should have number type');
    assertEqual(init.returnType.valueType, 'number', 'Return type should be number');
  });
  ...
  test('Parse ternary expression', () => {
    ...
    assertEqual(init.type, 'ConditionalExpression', 'Should be ConditionalExpression');
    assertEqual(init.test.type, 'Identifier', 'Test should be an Identifier');
    assertEqual(init.test.name, 'x', 'Test value should be "x"');
    assertEqual(init.consequent.value, 'positive', 'Consequent should be "positive"');
    assertEqual(init.alternate.value, 'negative', 'Alternate should be "negative"');
  });
  ...
  test('Parse simple ternary expression', () => {
    ...
    assertEqual(init.type, 'ConditionalExpression', 'Should be ConditionalExpression');
    assertEqual(init.test.type, 'BooleanLiteral', 'Test should be BooleanLiteral');
  ...
  test('Parse return statement', () => {
    ...
    assertEqual(fnBody.argument.type, 'NumericLiteral', 'Return argument should be NumericLiteral');
...
console.log('Running compiler tests for tokenizing and parsing...\n');

let tokenizeTestsPass = runTokenizerTests();
let parserTestsPass = runParserTests();

// Summary
console.log('\n=== Test Summary ===\n');
if (tokenizeTestsPass && parserTestsPass) {
  console.log(`${PASS} All tests passed!`);
} else {
  console.log(`${FAIL} Some tests failed. See above for details.`);
}